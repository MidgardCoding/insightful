...
Advanced JSON import will be implemented in future releases - for now, app creates a new package.json if one doesn't exist with a data template.

> *Right now, you can use the sample package provided with the MVP. Building the package manager UI is on the roadmap.*

## 📄 Package Format (limited to autogenerated package)

A minimal example (`taskmgr.json`):
